...
import { useMdProg006d0003Store } from './QueryPageStore';
import {
    getAxiosInstance,
    getProgItem,
...
const queryPageStore = useMdProg006d0003Store();
...
const krTypeName = (krType: string) => {
    const map: any = { INCREASE: 'Increase', DECREASE: 'Decrease', PERCENT: 'Percent', MILESTONE: 'Milestone', BINARY: 'Binary', MANUAL: 'Manual' };
    return map[krType] || krType;
};
...
const initQueryGridConfig = () => {
    return getGridConfig(
        ...
        [
            ...
            { label: 'KR Code', field: 'krCode' },
            { label: 'KR Name', field: 'krName' },
            { label: 'KR Type', field: 'krType' },
    ...
    try {
        ...
        const response = await axiosInstance.post(import.meta.env.VITE_API_URL + PageConstants.eventNamespace + '/findPage', {
            "field": {
                ...
                "krCodeLike"   : queryPageStore.queryParam.krCode,
                "krNameLike"   : queryPageStore.queryParam.krName,
                "krType"       : queryPageStore.queryParam.krType,
        ...
        if (response.data) {
            ...
            dsList.value = response.data.value.map((item: any) => ({
                ...item,
                objectiveOid: objectiveName(item.objectiveOid),
                krType: krTypeName(item.krType),
...
watch(
    () => queryPageStore.queryParam.cycleOid,
    () => {
        loadObjectiveList();
    }
);
...
<div class="row">
  <div class="col-12">
    <Toolbar
        ...
        description="OKR Key Result Query"
        refreshFlag="Y"
    ...
    <div class="row g-3">
      ...
      <div class="col-md-3">
        <div class="form-group form-floating">
          ...
          <label for="objectiveOid">Objective</label>
        </div>
      </div>
      <div class="col-md-2">
        <div class="form-group form-floating">
          <input type="text" class="form-control" id="krCode" placeholder="KR Code" v-model="queryPageStore.queryParam.krCode">
          <label for="krCode">KR Code</label>
        </div>
      </div>
      <div class="col-md-2">
        <div class="form-group form-floating">
          <input type="text" class="form-control" id="krName" placeholder="KR Name" v-model="queryPageStore.queryParam.krName">
          <label for="krName">KR Name</label>
        </div>
      </div>
      <div class="col-md-2">
        <div class="form-group form-floating">
          <select class="form-select" id="krType" v-model="queryPageStore.queryParam.krType">
            <option value="">All</option>
          ...
          <label for="krType">KR Type</label>
        </div>
      </div>
      <div class="col-md-2">
        <div class="form-group form-floating">
