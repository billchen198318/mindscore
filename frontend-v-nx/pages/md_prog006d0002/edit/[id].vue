...
const loadObjectiveList = async () => {
    ...
    try {
        ...
        if (response.data) {
            ...
            objectiveList.value = (response.data.value || []).filter((item: any) => item.oid !== formParam.value.oid);
        }
    } catch (e: any) {
        toast.warning(e?.message || e);
    }
...
const loadData = async () => {
    ...
    try {
        ...
        if (response.data) {
            ...
            formParam.value = {
                ...value.objective,
                parentOid: value.objective?.parentOid || pleaseSelectId,
...
<div class="row">
    <div class="col-12">
        <Toolbar
            ...
            description="OKR Objective Edit"
            refreshFlag="Y"
  ...
  <div class="card-body">
    <div class="row g-3">
      ...
      <div class="col-md-6">
        ...
        <select :class="['form-select', checkInvalid('parentOid', checkFields) ? 'is-invalid' : '']" id="parentOid" v-model="formParam.parentOid">
          <option :value="pleaseSelectId">{{ pleaseSelectLabel }}</option>
          <option v-for="item in objectiveList" :key="item.oid" :value="item.oid">{{ item.objectiveCode }} - {{ item.objectiveName }}</option>
        </select>
        <div v-if="checkInvalid('parentOid', checkFields)" class="invalid-feedback">{{ invalidFeedback('parentOid', checkFields) }}</div>
      </div>
      <div class="col-md-4">
