...
const loadData = async () => {
    ...
    try {
        ...
        hideLoading();
        if (response.data && response.data.success == import.meta.env.VITE_SUCCESS_FLAG) {
            formParam.value = { ...response.data.value, orgOid: response.data.value.orgOid || pleaseSelectId };
        } else {
  ...
  <div class="card-body">
    <div class="row g-3">
      ...
      <div class="col-md-6">
        ...
        <input type="text" class="form-control" id="account" v-model="formParam.account" readonly>
      </div>
      <div class="col-md-6">
