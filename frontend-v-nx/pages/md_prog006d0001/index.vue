...
import { useMdProg006d0001Store } from './QueryPageStore';
import {
    getAxiosInstance,
    getProgItem,
...
const queryPageStore = useMdProg006d0001Store();
...
const periodTypeName = (periodType: string) => {
    const map: any = { DAY: 'Day', WEEK: 'Week', MONTH: 'Month', QUARTER: 'Quarter', HALFYEAR: 'Half Year', YEAR: 'Year' };
...
const initQueryGridConfig = () => {
    return getGridConfig(
        ...
        [
            ...
            { label: 'Cycle Code', field: 'cycleCode' },
            { label: 'Cycle Name', field: 'cycleName' },
...
const btnQuery = async () => {
    ...
    try {
        ...
        const response = await axiosInstance.post(import.meta.env.VITE_API_URL + PageConstants.eventNamespace + '/findPage', {
            "field": {
                "cycleCodeLike" : queryPageStore.queryParam.cycleCode,
                "cycleNameLike" : queryPageStore.queryParam.cycleName,
        ...
        if (response.data) {
            ...
            dsList.value = response.data.value.map((item: any) => ({
                ...item,
                periodType: periodTypeName(item.periodType),
    ...
    <div class="row g-3">
      <div class="col-md-3">
        <div class="form-group form-floating">
          <input type="text" class="form-control" id="cycleCode" placeholder="Cycle Code" v-model="queryPageStore.queryParam.cycleCode">
          <label for="cycleCode">Cycle Code</label>
        </div>
      </div>
      <div class="col-md-3">
        <div class="form-group form-floating">
          <input type="text" class="form-control" id="cycleName" placeholder="Cycle Name" v-model="queryPageStore.queryParam.cycleName">
