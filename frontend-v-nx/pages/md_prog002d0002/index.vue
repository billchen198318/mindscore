...
import { useMdProg002d0002Store } from './QueryPageStore';
import {
    getAxiosInstance,
    getProgItem,
...
const queryPageStore = useMdProg002d0002Store();
const { showLoading, hideLoading, confirmFire } = useSwalLoading();

const pageProgramId = ref(PageConstants.QueryId);
const dsList = ref<any[]>([]);
const qFieldShow = ref(true);
const aggrTypeName = (aggrType: string) => aggrType === 'BUILTIN' ? '系統提供' : aggrType === 'CUSTOM' ? '使用者自訂' : aggrType;
...
const initQueryGridConfig = () => {
    return getGridConfig(
        ...
        [
            ...
            { label: '彙總代碼', field: 'aggrCode' },
            { label: '彙總名稱', field: 'aggrName' },
            { label: '類型', field: 'aggrType' },
...
const btnQuery = async () => {
    ...
    try {
        ...
        const response = await axiosInstance.post(import.meta.env.VITE_API_URL + PageConstants.eventNamespace + '/findPage', {
            "field": {
                "aggrCodeLike" : queryPageStore.queryParam.aggrCode,
                "aggrNameLike" : queryPageStore.queryParam.aggrName,
                "aggrType"     : queryPageStore.queryParam.aggrType,
        ...
        if (response.data) {
            ...
            dsList.value = response.data.value.map((item: any) => ({ ...item, aggrType: aggrTypeName(item.aggrType) }));
    ...
    <div class="row g-3">
      <div class="col-md-3">
        <div class="form-group form-floating">
          <input type="text" class="form-control" id="aggrCode" placeholder="彙總代碼" v-model="queryPageStore.queryParam.aggrCode">
          <label for="aggrCode">彙總代碼</label>
        </div>
      </div>
      <div class="col-md-3">
        <div class="form-group form-floating">
          <input type="text" class="form-control" id="aggrName" placeholder="彙總名稱" v-model="queryPageStore.queryParam.aggrName">
          <label for="aggrName">彙總名稱</label>
        </div>
      </div>
      <div class="col-md-3">
        <div class="form-group form-floating">
          <select class="form-select" id="aggrType" v-model="queryPageStore.queryParam.aggrType">
            <option value="">全部</option>
            <option value="BUILTIN">系統提供</option>
            <option value="CUSTOM">使用者自訂</option>
          </select>
          <label for="aggrType">彙總來源類型</label>
        </div>
      </div>
      <div class="col-md-3">
        <div class="form-group form-floating">
